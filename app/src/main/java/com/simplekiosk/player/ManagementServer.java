...
final class ManagementServer {
    ...
    private String buildHomePage() {
        ...
        html.append(".grid{display:grid;grid-template-columns:minmax(380px,1fr) minmax(430px,1.25fr);gap:12px;}@media(max-width:980px){.grid{grid-template-columns:1fr;}.schedule-row,.schedule-playlist-row{grid-template-columns:1fr!important;}.media-tools{grid-template-columns:1fr 1fr!important;}}");
        ...
        html.append(".media-tools{display:grid;grid-template-columns:1fr .7fr .55fr .65fr auto;gap:8px;align-items:end;margin:8px 0;}.media-actions{display:flex;flex-wrap:wrap;gap:6px;margin:8px 0;}table{width:100%;border-collapse:collapse;font-size:13px;}th,td{border-top:1px solid #303942;padding:7px;vertical-align:middle;text-align:left;}th{color:#aeb9c5;font-weight:normal;background:#151a20;position:sticky;top:0;}tr.risk-row{background:#25171a;}.thumb{width:64px;height:44px;object-fit:cover;background:#11161c;border:1px solid #3d4652;}.file-name{font-weight:bold;color:#fff;word-break:break-all;}.folder-cell{color:#b8c7d6;word-break:break-all;}.risk{color:#ffb3b3;font-weight:bold;}.ok{color:#9fe3b1;}.pager{display:flex;gap:8px;align-items:center;justify-content:flex-end;margin-top:8px;}");
        ...
        html.append("<section><h2>Upload media</h2><label>Target folder</label><input id='uploadFolder' placeholder='A-role or A-role/subset'><input id='file' type='file' multiple accept='image/png,image/jpeg,video/mp4'><button onclick='upload()'>Upload selected</button><progress id='progress' max='100' value='0'></progress><pre id='uploadResult'></pre></section>");
        html.append("<section><h2>Media library</h2><div class='toolbar-note'>Folders under /media are treated as groups. High-risk videos are warned but can still be used.</div><div class='media-tools'><div><label>Search</label><input id='mediaSearch' oninput='mediaPage=0;renderMedia()' placeholder='name or folder'></div><div><label>Folder</label><select id='mediaFolder' onchange='mediaPage=0;renderMedia()'></select></div><div><label>Type</label><select id='mediaType' onchange='mediaPage=0;renderMedia()'><option value='all'>all</option><option value='image'>image</option><option value='video'>video</option></select></div><div><label>Risk</label><select id='mediaRisk' onchange='mediaPage=0;renderMedia()'><option value='all'>all</option><option value='risk'>risk only</option><option value='ok'>ok only</option></select></div><button class='secondary' onclick='loadMedia()'>Refresh</button></div><div class='media-actions'><button class='secondary' onclick='batchAddSelected()'>Add selected</button><button class='secondary' onclick='addFolderToSchedule()'>Add folder</button><button class='danger' onclick='batchDeleteSelected()'>Delete selected</button></div><div id='mediaList'></div><div class='pager'><button class='secondary' onclick='mediaPage--;renderMedia()'>Prev</button><span id='mediaPageInfo'></span><button class='secondary' onclick='mediaPage++;renderMedia()'>Next</button></div><pre id='mediaResult'></pre></section>");
        ...
        html.append("var ACCESS_TOKEN='").append(accessToken).append("';var ACCESS_ON=").append(accessProtectionEnabled ? "true" : "false").append(";var config=defaultConfig();var mediaFiles=[];var mediaPage=0;var pageSize=25;var activeSchedule=-1;");
        ...
        html.append("function folders(){var m={'':true};for(var i=0;i<mediaFiles.length;i++)m[mediaFiles[i].folder||'']=true;var a=[];for(var k in m)a.push(k);a.sort();return a;}");
        html.append("function renderFolderFilter(){var el=document.getElementById('mediaFolder');var old=el.value||'all';var fs=folders();var h='<option value=all>all</option>';for(var i=0;i<fs.length;i++){var label=fs[i]||'(root)';h+=`<option value='${esc(fs[i])}'>${esc(label)}</option>`;}el.innerHTML=h;el.value=old;if(el.value!==old)el.value='all';}");
        html.append("function filteredMedia(){var q=(document.getElementById('mediaSearch').value||'').toLowerCase();var folder=document.getElementById('mediaFolder').value||'all';var type=document.getElementById('mediaType').value;var risk=document.getElementById('mediaRisk').value;var out=[];for(var i=0;i<mediaFiles.length;i++){var f=mediaFiles[i];var hay=((f.path||'')+' '+(f.folder||'')+' '+(f.name||'')).toLowerCase();if(q&&hay.indexOf(q)<0)continue;if(folder!='all'&&(f.folder||'')!=folder)continue;if(type!='all'&&f.type!=type)continue;var hasRisk=f.warnings&&f.warnings.length;if(risk=='risk'&&!hasRisk)continue;if(risk=='ok'&&hasRisk)continue;out.push({file:f,index:i});}return out;}");
        ...
        html.append("function renderMedia(){renderFolderFilter();var el=document.getElementById('mediaList');var list=filteredMedia();var pages=Math.max(1,Math.ceil(list.length/pageSize));if(mediaPage<0)mediaPage=0;if(mediaPage>=pages)mediaPage=pages-1;var start=mediaPage*pageSize;var page=list.slice(start,start+pageSize);document.getElementById('mediaPageInfo').textContent=(list.length?((start+1)+'-'+(start+page.length)):'0')+' / '+list.length;if(!mediaFiles.length){el.innerHTML='<div class=muted>No media files</div>';return;}if(!page.length){el.innerHTML='<div class=muted>No matching media</div>';return;}var h='<table><thead><tr><th></th><th>Preview</th><th>Folder</th><th>Name</th><th>Type</th><th>Size</th><th>Metadata</th><th>Status</th><th>Actions</th></tr></thead><tbody>';for(var p=0;p<page.length;p++){var f=page[p].file;var i=page[p].index;var warn=(f.warnings||[]);var addSchedule=activeSchedule>=0?`<button class=secondary onclick='addMediaToSchedule(${i})'>Add</button>`:'<button class=secondary disabled>Add</button>';h+=`<tr class='${warn.length?'risk-row':''}'><td><input class=media-check type=checkbox value='${i}'></td><td><img class=thumb loading=lazy src='${thumbForFile(f.path)}'></td><td class=folder-cell>${esc(f.folder||'(root)')}</td><td><div class=file-name>${esc(f.name||'')}</div><div class=muted>${esc(f.path||'')}</div></td><td>${esc(f.type||'')}</td><td>${esc(f.size||'')}</td><td>${esc(metadataText(f))}</td><td>${warn.length?'<span class=risk>'+esc(warn.join('; '))+'</span>':'<span class=ok>OK</span>'}${f.referenced?' <span class=pill>used</span>':''}</td><td>${addSchedule}<button class=secondary onclick='renameMedia(${i})'>Rename</button><button class=danger onclick='deleteMedia(${i})'>Delete</button></td></tr>`;}el.innerHTML=h+'</tbody></table>';}");
        ...
        html.append("function addFolderToSchedule(){var folder=document.getElementById('mediaFolder').value;if(!folder||folder=='all'){document.getElementById('mediaResult').textContent='Choose a folder first';return;}var list=filteredMedia();var count=0;for(var i=0;i<list.length;i++){if((list[i].file.folder||'')==folder){addMediaToSchedule(list[i].index);count++;}}document.getElementById('mediaResult').textContent='Added '+count+' folder items';}");

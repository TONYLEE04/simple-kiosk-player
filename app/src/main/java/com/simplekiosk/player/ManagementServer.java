...
final class ManagementServer {
    ...
    private String buildHomePage() {
        ...
        html.append("function reloadConfig(){fetch(api('/config')).then(r=>r.text()).then(t=>{try{config=JSON.parse(t);}catch(e){config=defaultConfig();document.getElementById('saveResult').textContent='No valid config loaded yet. Create schedules and save config.json.';}normalizeConfig();renderAccess();renderSchedules();renderMedia();preview();});}");
